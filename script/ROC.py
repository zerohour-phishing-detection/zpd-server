...
def effect(labeln, field, fielddelim, perfectval):
	...
	for row in results:
		...
		dataZ_text = np.append(dataZ_text, 0)
	...
	for row in results:
		...
		dataZ_image = np.append(dataZ_image, 0)
	...
	for row in results:
		...
		dataZ_both = np.append(dataZ_both, 0)
	...
	# Benign Data - that got confirmed before
	sqls = f"select distinct {field}, filepath from search_result_text  where filepath in (select distinct filepath from search_result_text where hit <> '') group by filepath"
	results = curs_b.execute(sqls).fetchall()
	for row in results:
		dataX_text = np.append(dataX_text, perfectval)
		dataY_text = np.append(dataY_text, 0)
		dataZ_text = np.append(dataZ_text, 1)
		
	sqls = f"select distinct {field}, filepath from search_result_image where filepath in (select distinct filepath from search_result_image where hit <> '') group by filepath"
	results = curs_b.execute(sqls).fetchall()
	for row in results:
		dataX_image = np.append(dataX_image, perfectval)
		dataY_image = np.append(dataY_image, 0)
		dataZ_image = np.append(dataZ_image, 1)
		
	sqls = f"select distinct {field}, filepath from search_result_image where filepath in (select distinct filepath from search_result_text where hit <> '' union select distinct filepath from search_result_image where hit <> '') union select distinct {field}, filepath from search_result_text  where filepath in (select distinct filepath from search_result_text where hit <> '' union select distinct filepath from search_result_image where hit <> '') group by filepath"
	results = curs_b.execute(sqls).fetchall()
	for row in results:
		dataX_both = np.append(dataX_both, perfectval)
		dataY_both = np.append(dataY_both, 0)
		dataZ_both = np.append(dataZ_both, 1)
	...
	#or use dataframe
	dataframeX_text = pd.DataFrame({'fieldval':dataX_text, 'confirmed_legit_prev':dataZ_text})
	dataframeX_image = pd.DataFrame({'fieldval':dataX_image, 'confirmed_legit_prev':dataZ_image})
	dataframeX_both = pd.DataFrame({'fieldval':dataX_both, 'confirmed_legit_prev':dataZ_both})
	...
	for lp in range(1000):
		X_train_text, X_test_text, y_train_text, y_test_text = train_test_split(dataframeX_text, dataY_text, test_size=0.2, random_state=lp)
		clf_reg_text = LogisticRegression(solver='liblinear')
		clf_reg_text.fit(X_train_text, y_train_text)
		y_score1 = clf_reg_text.predict_proba(X_test_text)[:,1]
		...
		if sc > auc1:
			...
			model_t = clf_reg_text
			data_x_text = X_test_text
			data_y_text = y_test_text

		X_train_image, X_test_image, y_train_image, y_test_image = train_test_split(dataframeX_image, dataY_image, test_size=0.2, random_state=lp)
		clf_reg_image = LogisticRegression(solver='liblinear')
		clf_reg_image.fit(X_train_image, y_train_image)
		y_score2 = clf_reg_image.predict_proba(X_test_image)[:,1]
		...
		if sc > auc2:
			...
			model_i = clf_reg_image
			data_x_image = X_test_image
			data_y_image = y_test_image
		
		
		X_train_both, X_test_both, y_train_both, y_test_both = train_test_split(dataframeX_both, dataY_both, test_size=0.2, random_state=lp)
		clf_reg_both = LogisticRegression(solver='liblinear')
		clf_reg_both.fit(X_train_both, y_train_both)
		y_score3 = clf_reg_both.predict_proba(X_test_both)[:,1]
...
effect("EMD", "min(emd)", "emd", 0.0)
#effect("EMD", "max(emd)", "emd", 1.0)
effect("DCT", "min(dct)", "dct", 0.0)
#effect("DCT", "max(dct)", "dct", 1.0)
effect("PSIM", "min(pixel_sim)", "pixel_sim", 0.0)
#effect("PSIM", "max(pixel_sim)", "pixel_sim", 1.0)
effect("SSIM", "min(structural_sim)", "structural_sim", 0.0)
#effect("SSIM", "max(structural_sim)", "structural_sim", 1.0)
effect("ORB", "min(orb)", "orb", 0.0)
