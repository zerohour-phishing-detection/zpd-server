...
def find_hits(tablename, region):
	...
	for searched_url in searched_url_list:
		...
		##	continue
		hit = None
		baseurl = conn_url.execute(f"select url from odp_urls where sha256 = '{searched_url[0]}'").fetchall()
		print(baseurl)
		baseurl = tldextract.extract(baseurl[0][0]).registered_domain

		if not region:
			searched_dns_list = cursor_hit.execute(f"select result, entry  from {tablename} where filepath = '{searched_url[0]}'").fetchall()
		else:
			searched_dns_list = cursor_hit.execute(f"select result, entry, region from {tablename} where filepath = '{searched_url[0]}' and region = 9999").fetchall()

		for url in searched_dns_list:
			url_fix = urlparse(url[0]).netloc
			if url_fix.endswith(baseurl):
				hit = url_fix
...
##print(f"Total: {len(all_hashes)}")
##print(f"Hits from image: {len(image_hits)} ({len(image_hits)/len(all_hashes)*100}%)")
##print(f"Hits from text: {len(text_hits)} ({len(text_hits)/len(all_hashes)*100}%)")
##print(f"Hits from image+text: {len(text_hits.union(image_hits))} ({len(text_hits.union(image_hits))/len(all_hashes)*100}%)")
##print(f"Hits from intersect image-text: {len(text_hits.union(image_hits)-text_hits)} ({len(text_hits.union(image_hits)-text_hits)/len(all_hashes)*100}%)")

##print(f"Hits only image had: {len(image_hits-text_hits)} ({len(image_hits-text_hits)/len(all_hashes)*100}%)")
##print(f"Hits only text had: {len(text_hits-image_hits)} ({len(text_hits-image_hits)/len(all_hashes)*100}%)")
