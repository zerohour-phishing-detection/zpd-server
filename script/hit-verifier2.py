		...
		for url in searched_dns_list:
			...
			for searchurl in list(chain.from_iterable(sanlist)):
				...
				if searchurl.endswith(baseurl):
					hit = searchurl
					print(f"hit for: {baseurl} and {searchurl}")
...
len_phishing = 947
print(f"Total: {510}")
print(f"Hits from image: {len(image_hits)} ({len(image_hits)/510*100}%)")
print(f"Hits from text: {len(text_hits)} ({len(text_hits)/510*100}%)")
print(f"Hits from image+text: {len(text_hits.union(image_hits))} ({len(text_hits.union(image_hits))/510*100}%)")
print(f"Hits from intersect image-text: {len(text_hits.union(image_hits)-text_hits)} ({len(text_hits.union(image_hits)-text_hits)/510*100}%)")

print(f"Hits only image had: {len(image_hits-text_hits)} ({len(image_hits-text_hits)/510*100}%)")
print(f"Hits only text had: {len(text_hits-image_hits)} ({len(text_hits-image_hits)/510*100}%)")
